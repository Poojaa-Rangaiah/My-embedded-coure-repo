...
void main(void) {
    ...
    while(1) {
        ...
        val = (hival<<8) + loval;                       //16 bit of Duty Cycle data
        volt = (val*338)/1023;                          //Calibrating voltage
        cmd(0x80);
        output(volt);
...
void init(void) {
    ...
    TRISA = 0x01;           //Configure PORTA as input
    ...
    CCP1CON = 0x3C;         //Random value only for MSB, configuring Duty cycle register
    CCPR1L = 0x2E;          //Random value
    T2CON = 0x06;           //Configuring time period register
    PR2 = 0x5E;             //Time period register
    cmd(0x38);
    delay(100);
    cmd(0x38);
    delay(100);
